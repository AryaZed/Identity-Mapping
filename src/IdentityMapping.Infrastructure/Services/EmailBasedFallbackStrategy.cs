...
{
    /// <summary>
    /// A fallback strategy that tries to find or create mappings based on email address
    /// </summary>
    public class EmailBasedFallbackStrategy : IFallbackStrategy
    {
        private readonly IUserIdentityRepository _userIdentityRepository;
        private readonly IUserIdMappingRepository _mappingRepository;
        private readonly ILogger<EmailBasedFallbackStrategy> _logger;

        public EmailBasedFallbackStrategy(
            IUserIdentityRepository userIdentityRepository,
            IUserIdMappingRepository mappingRepository,
            ILogger<EmailBasedFallbackStrategy> logger)
        {
        ...
        // Email-based strategy has medium priority
        public int Priority => 50;

        public bool CanHandle(string applicationId, FallbackContext context)
        {
            // This strategy can handle cases where we have an email address
            return !string.IsNullOrEmpty(context.UserEmail);
        }
        ...
        public async Task<FallbackResult> HandleAsync(string applicationId, FallbackContext context)
        {
            ...
            {
                return FallbackResult.CreateFailure("Email address is required for email-based fallback strategy");
            }

            _logger.LogInformation($"Attempting email-based fallback for application {applicationId} with email {context.UserEmail}");

            try
            {
                // Try to find an existing user identity by email
                var existingIdentity = await _userIdentityRepository.GetByEmailAsync(context.UserEmail);

                if (existingIdentity != null)
                {
                    _logger.LogInformation($"Found existing user identity with ID {existingIdentity.Id} for email {context.UserEmail}");
                    ...
                    if (string.IsNullOrEmpty(context.CentralizedIdentityId) && !string.IsNullOrEmpty(context.LegacyUserId))
                    {
                        ...
                        var newMapping = new UserIdMapping
                        {
                            ...
                            IsValidated = false,
                            Metadata = $"Created by email-based fallback strategy for email {context.UserEmail}"
                        };
                        ...
                        return FallbackResult.CreateSuccess(createdMapping, "Created new mapping based on existing user identity found by email");
                    }
                    ...
                    else if (!string.IsNullOrEmpty(context.CentralizedIdentityId) && string.IsNullOrEmpty(context.LegacyUserId))
                    {
                        if (existingIdentity.Id != context.CentralizedIdentityId)
                        {
                            ...
                            return FallbackResult.CreateFailure("Email matches a different centralized identity than provided");
                        }
                ...
                else if (!string.IsNullOrEmpty(context.UserDisplayName) && !string.IsNullOrEmpty(context.LegacyUserId))
                {
                    ...
                    _logger.LogInformation($"No existing user identity found for email {context.UserEmail}, creating new one");
                    ...
                    return FallbackResult.CreateSuccess(createdMapping, "Created new user identity and mapping based on email and display name");
                }
            ...
            {
                _logger.LogError(ex, "Error in email-based fallback strategy");
                return FallbackResult.CreateFailure($"Error in email-based fallback strategy: {ex.Message}");
            }
        }
    }
} 